import React from "react";
import Card from 'react-bootstrap/Card';
import Row from "react-bootstrap/Row";
import Col from "react-bootstrap/Col";
import {Link} from "react-router-dom";
import "./style1.css";  
import "bootstrap/dist/css/bootstrap.min.css";
import Bestseller1 from "../Images/bestseller1.png";
import Bestseller2 from "../Images/bestseller2.png";
function Bestseller() {
    return(<Row style={{width:"100%",paddingLeft:"5px"}} className="m-0">
    <Col style={{display:"flex",justifyContent:"space-around"}} lg={2} md={4} sm={4} xs={6}>
    <Card style={{ width: '11rem',border:"none"}}>
    <Card.Img variant="top" style={{width:"130px",height:"130px",margin:"auto",marginTop:"10px"}} src={Bestseller1} />
    <Card.Body>
    <Card.Text style={{fontSize:"13px"}}>
    Atomic Habits: An Easy and Proven Way to Build Good Habits and Break Bad Ones<br/>
    <b style={{fontSize:"11px"}}>Author Name/Publication</b>
    <br/>
    <div>Stars</div>
    <div>718 Ratings | 20 Reviews</div>
    <div style={{color:"#700303"}}><b>₹139</b></div>
    <div>FREE delivery: Monday, June 14</div>
    </Card.Text>
  </Card.Body>
</Card>
</Col>
    <Col style={{display:"flex",justifyContent:"space-around"}} lg={2} md={4} sm={4} xs={6}>
    <Card style={{ width: '11rem',border:"none"}}>
    <Card.Img variant="top" style={{width:"130px",height:"130px",margin:"auto",marginTop:"10px"}} src={Bestseller2}/>
    <Card.Body>
    <Card.Text style={{fontSize:"13px"}}>
    Atomic Habits: An Easy and Proven Way to Build Good Habits and Break Bad Ones<br/>
    <b style={{fontSize:"11px"}}>Author Name/Publication</b>
    <br/>
    <div>Stars</div>
    <div>718 Ratings | 20 Reviews</div>
    <div style={{color:"#700303"}}><b>₹139</b></div>
    <div>FREE delivery: Monday, June 14</div>
    </Card.Text>
  </Card.Body>
</Card>
</Col>
    <Col style={{display:"flex",justifyContent:"space-around"}} lg={2} md={4} sm={4} xs={6}>
    <Card style={{ width: '11rem',border:"none"}}>
    <Card.Img variant="top" style={{width:"130px",height:"130px",margin:"auto",marginTop:"10px"}} src={Bestseller1} />
    <Card.Body>
    <Card.Text style={{fontSize:"13px"}}>
    Atomic Habits: An Easy and Proven Way to Build Good Habits and Break Bad Ones<br/>
    <b style={{fontSize:"11px"}}>Author Name/Publication</b>
    <br/>
    <div>Stars</div>
    <div>718 Ratings | 20 Reviews</div>
    <div style={{color:"#700303"}}><b>₹139</b></div>
    <div>FREE delivery: Monday, June 14</div>
    </Card.Text>
  </Card.Body>
</Card>
</Col>
    <Col style={{display:"flex",justifyContent:"space-around"}} lg={2} md={4} sm={4} xs={6}>
    <Card style={{ width: '11rem',border:"none"}}>
    <Card.Img variant="top" style={{width:"130px",height:"130px",margin:"auto",marginTop:"10px"}} src={Bestseller2}/>
    <Card.Body>
    <Card.Text style={{fontSize:"13px"}}>
    Atomic Habits: An Easy and Proven Way to Build Good Habits and Break Bad Ones<br/>
    <b style={{fontSize:"11px"}}>Author Name/Publication</b>
    <br/>
    <div>Stars</div>
    <div>718 Ratings | 20 Reviews</div>
    <div style={{color:"#700303"}}><b>₹139</b></div>
    <div>FREE delivery: Monday, June 14</div>
    </Card.Text>
  </Card.Body>
</Card>
</Col>
    <Col style={{display:"flex",justifyContent:"space-around"}} lg={2} md={4} sm={4} xs={6}>
    <Card style={{ width: '11rem',border:"none"}}>
    <Card.Img variant="top" style={{width:"130px",height:"130px",margin:"auto",marginTop:"10px"}} src={Bestseller1} />
    <Card.Body>
    <Card.Text style={{fontSize:"13px"}}>
    Atomic Habits: An Easy and Proven Way to Build Good Habits and Break Bad Ones<br/>
    <b style={{fontSize:"11px"}}>Author Name/Publication</b>
    <br/>
    <div>Stars</div>
    <div>718 Ratings | 20 Reviews</div>
    <div style={{color:"#700303"}}><b>₹139</b></div>
    <div>FREE delivery: Monday, June 14</div>
    </Card.Text>
  </Card.Body>
</Card>
</Col>
    <Col style={{display:"flex",justifyContent:"space-around"}} lg={2} md={4} sm={4} xs={6}>
    <Card style={{ width: '11rem',border:"none"}}>
    <Card.Img variant="top" style={{width:"130px",height:"130px",margin:"auto",marginTop:"10px"}} src={Bestseller2}/>
    <Card.Body>
    <Card.Text style={{fontSize:"13px"}}>
    Atomic Habits: An Easy and Proven Way to Build Good Habits and Break Bad Ones<br/>
    <b style={{fontSize:"11px"}}>Author Name/Publication</b>
    <br/>
    <div>Stars</div>
    <div>718 Ratings | 20 Reviews</div>
    <div style={{color:"#700303"}}><b>₹139</b></div>
    <div>FREE delivery: Monday, June 14</div>
    </Card.Text>
  </Card.Body>
</Card>
</Col>
</Row>);
}
export default Bestseller;